...
template <typename C>
class container_buffer : public buffer<typename C::value_type> {
 private:
  C& container_;
 ...
 public:
  explicit container_buffer(C& c)
      : buffer<typename C::value_type>(c.size()), container_(c) {}
};

// Extracts a reference to the container from back_insert_iterator.
template <typename C>
inline C& get_container(std::back_insert_iterator<C> it) {
  using bi_iterator = std::back_insert_iterator<C>;
...
template <typename C>
struct is_contiguous_back_insert_iterator<std::back_insert_iterator<C>>
    : is_contiguous<C> {};
...
// GCC 8 and earlier cannot handle std::back_insert_iterator<C> with
...
template <typename C, typename S, typename... Args,
          FMT_ENABLE_IF(
              is_contiguous<C>::value&& internal::is_string<S>::value)>
inline std::back_insert_iterator<C> format_to(
    std::back_insert_iterator<C> out, const S& format_str,
    Args&&... args) {
  return vformat_to(
